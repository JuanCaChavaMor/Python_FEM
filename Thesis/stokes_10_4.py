...
class DesiredState(UserExpression):
  def eval(self, values, x):
    values[0] = (2*x[0] - 2*pow(x[0],2) + x[1] -14*x[0]*x[1] + 8*x[1]*pow(x[0],2) \
                  -pow(x[1],2) + 6*x[0]*pow(x[1],2))
##### Set data #####
initRef = 20											#initial refinement\

##### define rhs, bc and mesh  #####
g =  Constant(1.0)
Dbc= Constant((0.0,0.0))
...
bc = DirichletBC(X.sub(1),X.sub(2),Dbc,'on_boundary')       # Check vector valued  BC
