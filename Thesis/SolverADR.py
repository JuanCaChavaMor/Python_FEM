...
def SolverADR(mesh,PU,PY,beta,phi,yd,g,lam):
    ### Parameters
    epsi =Constant(1.0)
    kappa=Constant(1.0)
    be=Constant((3.0, 4.0))
    ...
    # beta=interpolate(beta,U)
    # phi=interpolate(phi,U)
    # yd=interpolate(yd,Y)
    # g=interpolate(g,U)
    ...
    ## Bilinear Forms
    Co= (beta*p*w+u*w)*dx
    Ay= (inner(epsi*grad(y),grad(v))+(inner(be,grad(y)))*v+kappa*y*v-u*v)*dx
    Ap= (inner(epsi*grad(p),grad(q))-(inner(be,grad(p)))*q+kappa*p*q-y*q)*dx
    a = Co+Ay+Ap
    L = (phi*w+g*v-yd*q)*dx
    ## Assign the solution
    uh_yh_ph = Function(X)
